...
<!-- Section heading -->
<div class="container ">
 <h3 class="h1-responsive font-weight-bold my-5 border-bottom team-sec-title font-2 py-3">CREATE EPISODE</h2>
</div>
...
@csrf
    ...
    <div class="form-group m-5">
        <input type="text" class="form-control {{ $errors->has('name') ? 'has-error' : '' }}" id="InputTitle" name="episodeTitle" placeholder="title" value="{{ old('episodeTtile') }}">
    </div>
    ...
    <div class="form-group m-5">
        <select id="inputRole" class="form-control font-color-black {{ $errors->has('role_id') ? 'has-error' : '' }}" name="episodeTopic">
            ...
            <option class="font-color-black" value="{{$topic->id}}">{{$topic->name}}</option>
            @empty
